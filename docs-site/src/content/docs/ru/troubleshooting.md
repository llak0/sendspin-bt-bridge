...
**Фикс, подтверждённый отчётом сообщества ([#212](https://github.com/trudenboy/sendspin-bt-bridge/issues/212)).** Перевести Pi на 5 GHz WiFi-сеть, если роутер это поддерживает:
...
1. **Settings → Devices & services → Bluetooth → ⋮ → Disable** (или **Delete**, если интеграция вообще не нужна).
2. В UI бриджа: **Configuration → Bluetooth → Scan nearby**.
3. Запарить колонку как обычно.

Если HA Bluetooth-интеграция нужна для других устройств (ESPHome BLE-прокси, room-presence сенсоры, счётчики), оптимальная схема — **два адаптера**: встроенный оставить за HA, а к бриджу подключить USB BT-донгл. Подробности — [Bluetooth adapters](/sendspin-bt-bridge/ru/bluetooth-adapters/).
